...
var rotateMatrix = function(matrix) {
  ...
  // m = the number of rows in our matrix = number of nested arrays
  var m = matrix.length;
  // n = the number of columns in our matrix = length of nested arrays
  var n = matrix[0].length;

  // Instantiate an array to hold our new, rotated matrix
  // Prime it to have the appropriate number of nested arrays = n
  var rotatedMatrix = [];
  for (var i = 0; i < n; i++) {
    rotatedMatrix[i] = [];
    // inner for loop to access i, j of the rotatedMatrix,
    // where i = row index, j = column index
    for (var j = 0; j < m; j++) {
      // recall how to define each element, rotatedMatrix[i][j]...
